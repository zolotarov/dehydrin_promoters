...
with open(argv[1], 'r') as input_file:
	for line in input_file:
		if ">" in line or "Sequence" in line:
			continue
		else:
			all_lines += line.strip()

all_lines = all_lines.upper().replace('N','').replace('R','').replace('Y','').replace('S','').replace('W','').replace('K','').replace('M','').replace('B','').replace('D','').replace('H','').replace('V','')
